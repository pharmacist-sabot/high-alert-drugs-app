...
async function fetchDrugDetail() {
  ...
  try {
    ...
    // ดึงข้อมูล *ทุกคอลัมน์* จากยา ID ที่ระบุ
    const { data, error: dbError } = await supabase
      .from('high_alert_drugs')
      .select('*') // <-- ดึงทั้งหมด
      .eq('id', props.drugId)
      .single() // <-- เราคาดหวังผลลัพธ์แค่ 1 รายการ
...
// เมื่อ drugId ที่รับเข้ามาเปลี่ยนค่า (เช่น ผู้ใช้เปิดดูยาตัวอื่นต่อ) ให้ดึงข้อมูลใหม่
...
.modal-overlay {
  ...
  width: 100%;
  height: 100%;
  background: rgba(30, 41, 59, 0.5); /* สีพื้นหลังเข้มขึ้นเล็กน้อย */
  backdrop-filter: blur(4px); /* เพิ่มความสวยงาม */
  display: flex;
  align-items: center;
  justify-content: center;
  z-index: 1000;
  padding: 1rem; /* เพิ่ม padding เผื่อจอเล็กมากๆ */
...
/* --- 2. Base Card Structure (The Fix is Here) --- */
/* นำโครงสร้างหลักมาจาก DrugSearch.vue และปรับแก้เพื่อแก้ปัญหา Scroll */
.drug-card {
  width: 26rem;
  max-width: 100%; /* ให้ยืดหยุ่นใน overlay */
  height: 90vh;    /* ใช้ height แทน max-height/min-height เพื่อให้ flexbox คำนวณได้ง่าย */
  max-height: 700px; /* จำกัดความสูงสูงสุดบนจอใหญ่ */
  ...
  overflow: hidden; /* สำคัญมาก: ซ่อนส่วนที่เกินจากกรอบหลัก */
}

/* --- 3. Card Header (ปรับสำหรับ Modal) --- */
.card-header {
  background: #0066b3;
  color: #fff;
...
.close-btn {
  ...
  align-self: flex-start; /* จัดให้อยู่มุมบน */
}
.close-btn:hover {
  opacity: 1;
  transform: scale(1.1);
...
/* --- 4. Card Content (ส่วนที่ Scroll ได้) --- */
.card-content {
  /* --- The Magic Lines for Scrolling --- */
  flex: 1 1 auto; /* อนุญาตให้ยืดและหดได้ */
  min-height: 0;  /* อนุญาตให้ element เล็กลงกว่าเนื้อหาได้ */
  overflow-y: auto; /* ทำให้ Scrollbar ปรากฏเมื่อเนื้อหาล้น */
...
.status {
  ...
  font-size: 1.06rem;
  gap: 1rem;
  height: 100%; /* ทำให้ spinner อยู่กลางจอ */
}
.status-error { color: #c0392b; font-weight: 600; }

.spinner {
  width: 2.4em; height: 2.4em;
  border: .32em solid #d0e6f9;
  border-top: .32em solid #0066b3;
  border-radius: 50%;
  animation: spin 1s linear infinite;
}
@keyframes spin { to { transform: rotate(360deg); } }

/* --- 6. Styles for Detail Sections --- */
.details-grid {
  display: flex;
  flex-direction: column;
  gap: 1.2rem; /* เพิ่ม gap เล็กน้อย */
}
...
.detail-section h3 {
  margin: 0 0 .6em 0;
  font-size: 1rem;
  font-weight: 700; /* หนาขึ้นเล็กน้อย */
  color: #005ea7;
...
@media (min-width: 600px) {
  .drug-card {
    width: 30rem; /* ขยายขนาดบนจอใหญ่ขึ้น */
  }
